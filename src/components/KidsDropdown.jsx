  ...
  return <Menu as ='div' className="w-full h-full bg-white  relative">
         {/* btn */}
          ...
          <MenuItems as ='ul' className='bg-white absolute w-full flex flex-col z-40' >
            {list.map((li, i) =>{
              return <MenuItem onClick={()=> setKids(li.name)} as='li' key={i} className='border-b last-of-type:border-b-0 h-12 hover:bg-accent hover:text-white w-full flex justify-center items-center cursor-pointer' >
                {li.name}
              </MenuItem>
            })}
          </MenuItems>
